...
void hu_ssl_inf_log() {
  ...
  const char *ssl_state_string_long =
      SSL_state_string_long(hu_ssl_ssl); // "SSLv3 write client hello B"
...
//#ifdef __i386__
//#ifdef __arm__
int internalCertificateVerificationCallback(int preverify_ok,
                                            X509_STORE_CTX *x509_ctx) {
  // preverify_ok contains 1 if the pre-verification succeeded, 0 otherwise.
  logd("XLA in internalCertificateVerificationCallback");
...
int hu_ssl_handshake() {
  ...
  // PEM_read_bio_PrivateKey(bp, NULL, pass_cb,  "My Private Key");
  // Read a private key from a BIO using the pass phrase "hello":   key =
  ...
  while (!hs_finished && hs_ctr++ < 2) {
    ...
    if (ena_log_verbo ||
        (SSL_get_error(hu_ssl_ssl, ret) != SSL_ERROR_WANT_READ)) {
      hu_ssl_ret_log(ret);
      hu_ssl_inf_log();
    }

    ret =
        BIO_read(hu_ssl_wm_bio, &hs_buf[6],
                 sizeof(hs_buf) -
    ...
    ret =
        hu_aap_tra_set(AA_CH_CTR, 3, 3, hs_buf,
                       len); // chan:0/AA_CH_CTR   flags:first+last msg_type:SSL
    if (ret <= 0 || ret != len) {
      loge("hu_aap_tra_set() req ret: %d  len: %d", ret, len);
    }
    ret = hu_aap_tra_send(0, hs_buf, len,
                          2000); // Send Client request to AA Server
    if (ret <= 0 || ret != len) {
      loge("hu_aap_tra_send() HS client req ret: %d  len: %d", ret, len);
    }

    ret = hu_aap_tra_recv(hs_buf, sizeof(hs_buf), -2); // Get Rx packet from
                                                       // Transport: Receive
                                                       // Server response:
                                                       // Hello/Change Cipher
  ...
  hs_finished =
      1; // !!!! SSL_is_init_finished() does not work for some reason !!!
